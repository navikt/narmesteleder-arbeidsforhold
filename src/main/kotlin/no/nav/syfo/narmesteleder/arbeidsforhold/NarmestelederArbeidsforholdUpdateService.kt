...
class NarmestelederArbeidsforholdUpdateService(
    ...
    suspend fun updateNarmesteledere() {
        narmestelederDb.use {
            ...
            val checkedNarmesteleder: List<CheckedNarmesteleder> =
                narmesteleder
                    .chunked(100)
                    .map {
                        GlobalScope.async(context = Dispatchers.Fixed) { checkNarmesteleder(it) }
                    }
                    .awaitAll()
                    .flatten()
    ...
    private suspend fun checkNarmesteleder(it: List<NarmestelederDbModel>) =
        it.mapNotNull {
            try {
                ...
                when (checkedNarmesteleder.valid) {
                    true -> {
                        ...
                        narmestelederKafkaProducer.sendNlAvbrutt(
                            checkedNarmesteleder.narmestelederDbModel
                        )
